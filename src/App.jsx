...
function GoogleReviewsBlock() {
  ...
  const scrollReviews = (direction = "next") => {
    ...
    const cardWidth = sliderRef.current.querySelector("article")?.offsetWidth || 340;
    const gap = 24;
    ...
    <section className="mx-auto mt-20 max-w-7xl px-5">
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl md:p-10">
        ...
        <div className="relative">
          ...
          <div
            ref={sliderRef}
            className="no-scrollbar flex snap-x snap-mandatory gap-6 overflow-x-auto scroll-smooth px-1 py-3 md:px-14"
          >
            {reviews.map((review, index) => (
              <article
                key={index}
                className="flex h-[275px] w-[85vw] shrink-0 snap-center flex-col justify-between rounded-3xl border border-white/10 bg-[#0b132b]/90 p-6 shadow-lg transition duration-300 hover:-translate-y-2 hover:border-orange-300/50 hover:shadow-orange-500/20 sm:w-[360px] md:w-[380px]"
              >
                <div>
