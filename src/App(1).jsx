    ...
    <main className="min-h-screen bg-[#0b132b] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#071027]/90 shadow-2xl shadow-black/20 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3">
          ...
          <div className="pointer-events-none absolute left-1/2 hidden -translate-x-1/2 text-center lg:block xl:hidden">
            <p className="font-[family-name:var(--logo-font)] text-3xl leading-none tracking-normal text-white">
     ...
     <section className="relative min-h-[78vh] overflow-hidden px-5 py-20 text-center">
  {/* Fond premium */}
  <div className="absolute inset-0 bg-[#050816]" />

  {/* Montagnes / profondeur */}
  <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#1c2541_0%,#0b132b_42%,#050816_100%)] opacity-95" />

  {/* Flamme organique gauche */}
  <div className="pointer-events-none absolute -left-20 top-24 h-72 w-72 animate-[flameFloat_4.5s_ease-in-out_infinite] rounded-full bg-[radial-gradient(circle,#f77f00_0%,#ffb703_28%,#1b4332_58%,transparent_72%)] opacity-20 blur-3xl" />

  {/* Glow central */}
  <div className="pointer-events-none absolute left-1/2 top-28 h-80 w-80 -translate-x-1/2 animate-[flameBreath_5s_ease-in-out_infinite] rounded-full bg-[#f77f00]/10 blur-3xl" />

  {/* Contenu */}
  <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center justify-center pt-8">
    <img
      src="/Logoweb.png"
      alt="Logo Tout Feu Tout Flamme"
      className="mb-7 h-28 w-28 animate-[premiumReveal_1s_ease-out_forwards] object-contain opacity-0 drop-shadow-[0_0_28px_rgba(247,127,0,0.5)]"
    />

    <p className="mb-4 animate-[premiumReveal_1s_ease-out_0.1s_forwards] rounded-full border border-white/10 bg-white/5 px-5 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-white/70 opacity-0 backdrop-blur-md">
      Entretien • Ramonage • Dépannage
    </p>

    <h1 className="animate-[premiumReveal_1s_ease-out_0.2s_forwards]  text-3xl font-bold tracking-wide text-white opacity-0 md:text-6xl">
            TOUT FEU TOUT FLAMME
    </h1>

    <p className="mt-5 animate-[premiumReveal_1s_ease-out_0.35s_forwards] text-lg font-medium text-white/80 opacity-0 md:text-2xl">
      Expert en poêle à granulés toutes marques
    </p>

    <p className="mt-4 max-w-2xl animate-[premiumReveal_1s_ease-out_0.5s_forwards] text-base leading-relaxed text-white/60 opacity-0 md:text-lg">
      L’exigence artisanale. La tranquillité toute l’année.
    </p>

    <div className="mt-8 h-[2px] w-36 animate-[premiumReveal_1s_ease-out_0.65s_forwards] bg-gradient-to-r from-transparent via-[#f77f00] to-transparent opacity-0" />

    <div className="mt-10 flex animate-[premiumReveal_1s_ease-out_0.8s_forwards] flex-col gap-4 opacity-0 sm:flex-row">
      <a
        href="#offres"
        className="rounded-full bg-[#f77f00] px-8 py-4 text-sm font-bold uppercase tracking-wide text-white shadow-lg shadow-orange-500/25 transition hover:-translate-y-1 hover:bg-[#ff8f1f]"
      >
        Voir les contrats
      </a>
      ...
      <a
        href="#contact"
        className="rounded-full border border-white/15 bg-white/5 px-8 py-4 text-sm font-bold uppercase tracking-wide text-white/85 backdrop-blur-md transition hover:-translate-y-1 hover:bg-white/10"
      >
        Demander un conseil
      </a>
    </div>
...
function GoogleReviews() {
  ...
  const fallbackReviews = [
    {
      text: { text: "Sérieux et efficace ! Travail propre et surtout bien fait 👍 Je recommande ++ …" },
      authorAttribution: { displayName: "Christan T." },
    },
    {
      text: { text: "Entretien impeccable de mon poêle 🔥 Je recommande vivement Benjamin : très professionnel et à l’écoute. Il a remis mon poêle à neuf, avec un nettoyage complet et minutieux. …" },
      authorAttribution: { displayName: "Amélie G." },
    },
    {
      text: { text: "Entretien du poêle fait en temps et en heure, avec beaucoup de professionnalisme, du respect de la propreté de la maison. Rien à redire, si, de ne pas hésiter à faire appel à ses services. Il sera là pour le contrôle 2027." },
      authorAttribution: { displayName: "Pierre A." },
    },
    {
      text: { text: "Tres pro, vraiment efficace, le poele fonctionne a nouveau comme neuf apres un bon nettoyage annuel" },
      authorAttribution: { displayName: "Pascal L." },
    },
    {
      text: { text: "Intervention rapide Intervention de qualité Personne très professionnel et efficace et en plus il m’a laisser la maison nickel alors que mon poêle été encrassé ! Je recommande +++" },
      authorAttribution: { displayName: "Amandine P." },
    },
  ];
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/api/google-reviews`)
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error("Google reviews unavailable"))))
      .then(setData)
      .catch(() => setData(null));
  }, []);
  ...
  const totalReviews = data?.totalReviews ? `${data.totalReviews} avis Google` : "Avis clients Google";
  const googleReviewsUrl = data?.googleUrl || "https://www.google.com/maps/place/Benjamin+Plessis+-+tout+feu+tout+flamme-+Ramonage+%26+Entretien+Po%C3%AAles+Granul%C3%A9s+Comminges./@44.6268777,-1.2949243,799003m/data=!3m1!1e3!4m6!3m5!1s0x2740791fa84cb45:0x7babd4d8fca81ba9!8m2!3d43.130423!4d0.731989!16s%2Fg%2F11yycsrjmq?entry=ttu&g_ep=EgoyMDI2MDQyMi4wIKXMDSoASAFQAw%3D%3D";
  const writeReviewUrl = data?.writeReviewUrl || "https://g.page/r/CakbqPzY1Kt7EAE/review";

  return (
    <section id="avis" className="relative overflow-hidden bg-[#050814] px-5 py-20 text-white md:py-28">
      <div className="absolute left-1/2 top-1/2 h-[420px] w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#f77f00]/10 blur-3xl" />
      <div className="absolute right-[-120px] top-10 h-[280px] w-[280px] rounded-full bg-[#3a86ff]/10 blur-3xl" />
      <div className="absolute bottom-[-160px] left-[-120px] h-[320px] w-[320px] rounded-full bg-[#4cc9f0]/10 blur-3xl" />

      <div className="relative mx-auto max-w-6xl">
        <div className="mb-10 text-center md:mb-14">
          <p className="mb-3 text-xs font-black uppercase tracking-[0.35em] text-[#f77f00] md:text-sm">
            Avis Google
          </p>

          <h2 className="font-serif text-4xl font-black text-white md:text-6xl">
            Ils nous font confiance
          </h2>

          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-white/60 md:text-base">
            Des interventions propres, sérieuses et durables pour votre poêle à granulés.
          </p>
        </div>

        <div className="mx-auto mb-8 max-w-xl rounded-[2rem] border border-[#f77f00]/20 bg-white/[0.04] p-6 text-center shadow-2xl shadow-[#f77f00]/10 backdrop-blur md:p-8">
          <div className="text-5xl font-black md:text-6xl">{rating}</div>
          <div className="my-3 text-2xl text-[#f77f00] md:text-3xl">★★★★★</div>
          <p className="text-sm font-semibold text-white/55">{totalReviews}</p>
        </div>

        <div className="mx-auto max-w-3xl rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl md:p-10">
          <div
            className={`transition-all duration-500 ${
              fade ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0"
            }`}
          >
            <div className="mb-5 text-center text-xl text-[#f77f00]">★★★★★</div>

            <p className="text-center text-lg leading-relaxed text-white/85 md:text-2xl">
              “{review?.text?.text || "Chargement des avis Google..."}”
            </p>
            ...
            <div className="mt-8 text-center">
              ...
              <p className="mt-1 text-sm text-white/45">Avis Google</p>
            </div>
          </div>
        </div>
          ...
          {reviews.map((_, i) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all ${
                i === index
                  ? "w-8 bg-[#f77f00] shadow-lg shadow-[#f77f00]/40"
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full rounded-full bg-[#f77f00] px-8 py-4 text-center font-black text-white shadow-lg shadow-[#f77f00]/25 transition hover:-translate-y-0.5 hover:bg-[#ff8c1a] sm:w-auto"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full rounded-full border border-white/15 bg-white/[0.04] px-8 py-4 text-center font-bold text-white transition hover:-translate-y-0.5 hover:bg-white/10 sm:w-auto"
          >
            Laisser un avis
          </a>
        </div>
